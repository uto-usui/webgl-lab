...
export class DebugUi {
  ...
  constructor() {
     ...
     * Debug
     */
    this.pane = new Tweakpane({ title: 'Parameters' })
    this.pane.addInput(this.mesh.position, 'y', {
      min: 0,
      max: 10,
      step: 0.01,
      label: 'elevation',
    })
    this.pane.addInput(this.mesh, 'visible')
    this.pane.addInput(material, 'wireframe')
    this.pane
      .addInput(parameters, 'color', { input: 'color' })
      .on('change', () => {
        material.color.set(parameters.color)
      })

    this.pane.addInput(parameters, 'spin').on('change', (event) => {
      gsap.to(this.mesh.rotation, 1, {
        y: this.mesh.rotation.y + Math.PI * 2,
      })
      console.log('📌 spin change', event)
    })
